...
//        String text = message.getSendMessage().getName();
...
//    }
    ...
    public Message insert(String text, String photo, int keyboardId) throws SQLException {
        /*ID	TEXT	PHOTO	KEYBOARD_ID*/
        PreparedStatement ps = connection.prepareStatement("INSERT INTO Message VALUES(default, ?, ?, ?)");
        ps.setString(1, text);
        ps.setString(2, photo);
        ps.setInt(3, keyboardId);
        ps.execute();
        ResultSet rs = ps.getGeneratedKeys();
        rs.next();
        return getMessage(rs.getInt(1));
    }
}
